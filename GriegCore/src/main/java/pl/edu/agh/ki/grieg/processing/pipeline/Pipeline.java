...
public class Pipeline<T> implements Iteratee<T> {
    ...
    /**
     * Checks the node name against the {@link #NAME_PATTERN}, and if it is
     * valid, and no node with such name has been previously registered, puts it
     * into internal node map. Otherwise, throws an
     ...
     *             If the name is invalid, or if there is already a node with
     *             such name
     */
    private void addNode(String name, Node node) {
        if (NAME_PATTERN.matcher(name).matches()) {
            if (nodes.containsKey(name)) {
                String msg = String.format("Node '%s' already exists", name);
                throw new IllegalArgumentException(msg);
            } else {
                nodes.put(name, node);
            }
        } else {
            String msg = String.format("'%s' is not a legal node name", name);
            throw new IllegalArgumentException(msg);
        }
    }
